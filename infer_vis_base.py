...
def save_book_predictions_summary(inferencer_obj, bookname, all_image_paths, output_root_dir):
    ...
    current_prompts_for_summary = ["person", "illustration", "polearm"]
...
for img_path in all_image_paths:
    ...
    base_predictions_full = base_inferencer_full.predict(
        str(img_path),
        text_prompts=["person", "illustration", "polearm"]
    )

    output_path_full = base_all_output_dir / f"{img_path.stem}_base_multi.png"
    base_inferencer_full.visualize(base_predictions_full, str(output_path_full))

    print(f"✓ Processed {img_path.name}")
    # Print summary
    if isinstance(base_predictions_full, dict):
        for result in base_predictions_full.values():
...
print(f"\nBase model inference for all images complete. Visualizations saved to {base_all_output_dir}")
