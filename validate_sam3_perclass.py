...
def validate(config_path, weights_path, val_data_dir, num_samples=None,
    ...
    if filter_classes:
        eval_classes = [(name, cat_id)
                        for cat_id, name in val_ds.categories.items()
                        if name.lower() in {c.lower() for c in filter_classes}]
    else:
        eval_classes = [('object', 1)]  # original single-class behaviour

    # Compute metrics per class
    for cls_name, cls_cat_id in eval_classes:
        print("\n" + "="*80)
        print(f"COMPUTING METRICS FOR CLASS: {cls_name.upper()} (cat_id={cls_cat_id})")
        print("="*80)

        # Filter predictions to this class
        cls_predictions = [p for p in all_predictions if p.get('category_id', 1) == cls_cat_id]
        cls_image_ids   = [all_image_ids[i] for i, p in enumerate(all_predictions)
                           if p.get('category_id', 1) == cls_cat_id]

        # Build GT filtered to this class only
        # Temporarily mask _img_obj_cat_ids to only include this cat_id
        orig_img_obj_cat_ids = val_ds._img_obj_cat_ids
        val_ds._img_obj_cat_ids = {
            idx: {obj_id: cid for obj_id, cid in obj_map.items() if cid == cls_cat_id}
            for idx, obj_map in orig_img_obj_cat_ids.items()
        }
        orig_categories = val_ds.categories
        val_ds.categories = {cls_cat_id: cls_name}
        ...
        val_ds.categories = orig_categories
        val_ds._img_obj_cat_ids = orig_img_obj_cat_ids
        ...
        if len(coco_predictions) == 0:
            print(f"\n[ERROR] No predictions for class {cls_name}. Skipping.")
            continue

        import tempfile, shutil
        temp_dir = tempfile.mkdtemp(prefix=f"sam3_eval_{cls_name}_")
        ...
        # Compute mAP
        print("\n" + "="*80)
        print(f"COCO mAP EVALUATION — {cls_name}")
        print("="*80)
        ...
        # Compute cgF1
        print("\n" + "="*80)
        print(f"cgF1 EVALUATION — {cls_name}")
        print("="*80)
