    ...
    for item in predictions:
        ...
        for il_idx, (il_mask, il_score, il_box) in enumerate(all_illustrations):
            ...
            for hi, (h_mask, h_score, h_box) in enumerate(humans_in):
                ...
                # Dynamic padding based on human height
                h_height = h_box[3] - h_box[1]
                dynamic_padding = max(5, int(h_height * 0.1))
                ...
                if is_armed:
                    ...
                    result = crop_rgba(img_array, combined_mask, dynamic_padding)
                    if result is None:
                        continue
                    result.save(str(armed_dir / f"{stem}_il{il_idx+1}_hm{hi+1}_armed.png"))
                    stats["armed"] += 1
                else:
                    result = crop_rgba(img_array, h_mask, dynamic_padding)
                    if result is None:
                        continue
