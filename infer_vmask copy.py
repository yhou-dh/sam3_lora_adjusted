        ...
        if isinstance(predictions, dict):
            for prompt, result_dict in predictions.items():
                ...
                image_data["detections"].append({
                    ...
                    "masks_rle":      masks_rle,   # compact RLE, decode with pycocotools
                })
        else:
...
booknames = sorted([d.name for d in BOOK_DIR.iterdir() if d.is_dir()])
...
for bookname in booknames:
    ...
    if not image_dir.exists():
        print(f"  ⚠ Folder not found for {bookname}, skipping.")
        continue
